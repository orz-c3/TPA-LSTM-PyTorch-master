...
class Model(nn.Module):
    def __init__(self, args, data):
        ...
        torch.nn.init.xavier_uniform_(self.attention_matrix_uni_lstm)
        torch.nn.init.xavier_uniform_(self.context_vector_matrix_uni_lstm)
        torch.nn.init.xavier_uniform_(self.final_hidden_uni_matrix)
        torch.nn.init.xavier_uniform_(self.final_state_matrix)
        torch.nn.init.xavier_uniform_(self.bridge_matrix)
        ...
        if (args.output_fun == 'tanh'):
            self.output = F.tanh;
        # 在模型初始化中定义新的全连接层
        self.prediction_layer = nn.Linear(self.original_columns, 9)
    ...
    Outputs: output, (h_n, c_n)
        ...
        for feature_num in range(0, self.original_columns):
            ...
            attention_score = torch.nn.functional.softmax(mat2, dim=1)  # 使用softmax替代sigmoid
        ...
        if (self.hw > 0):
            ...
            # 使用全连接层来获取前九个特征的预测值
        predictions = self.prediction_layer(res.view(batch_size, -1))
